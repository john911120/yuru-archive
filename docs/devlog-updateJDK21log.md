...
####🔧 Spring Boot / Dependency Plugin 更新対応の理由整理
#■ 対応背景
JDK 21 への移行および Spring Security 6.x の deprecated API 対応を進める中で、
プロジェクト全体の警告数が大幅に減少し、設定・挙動ともに安定した状態に到達したため、あわせて Spring Boot のパッチバージョン更新を実施しました。

#■ 更新内容
Spring Boot 3.5.6 → 3.5.9（パッチバージョンアップ）
io.spring.dependency-management 1.1.4 → 1.1.7

#■ 更新理由
① Spring Boot パッチ更新（3.5.6 → 3.5.9）
パッチバージョンは バグ修正・安定性向上・セキュリティ修正が主目的で、
既存の API や挙動に大きな変更はなく、破壊的変更のリスクが低い。
SecurityConfig を含む主要な deprecated 対応が完了しており、アップデート耐性が高い状態であったため、このタイミングで更新を実施させております。

② dependency-management プラグイン更新（1.1.4 → 1.1.7）
Spring Boot 3.x 環境との 互換性・安定性向上とGradle / 依存関係解決に関する内部改善が中心になってプロジェクトの依存関係管理を最新状態に揃えることで、将来的なアップグレード時の差分・調査コストを低減することに役に立ちます。

#■ 改善効果
ビルド警告の削減と依存関係の安定化
将来の Spring Boot / Spring Security 更新に備えた 地雷除去
バージョン更新に対する心理的・技術的コストの低減

#■ 補足
本更新は 機能追加や挙動変更を目的としたものではなく、既存機能を維持したまま、内部基盤を最新・安定状態に揃えるための対応です。

#■ まとめ
事前に deprecated 対応と警告整理を完了させた上で、リスクの低いパッチ更新を行い、プロジェクト全体の安定性を一段引き上げました。
