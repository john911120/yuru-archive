...
## Spring Boot 3.5.15アップデート後のThymeleaf互換性対応
Spring Boot 3.5.15へのアップデート後、質問詳細画面を表示した際に、Thymeleafのテンプレート解析エラーが発生しました。
...
エラーは `question_detail.html` 内で、ThymeleafテンプレートからSpring Beanを直接呼び出していた箇所で発生していました。
...
Spring BootおよびThymeleafのアップデートに伴い、テンプレート式に対するセキュリティ制限が強化され、従来のBean直接呼び出しが拒否されたことが原因です。

### 対応内容

MarkdownからHTMLへの変換処理をテンプレート側からController側へ移動しました。

Controllerでは、回答IDをキーとしたMapに変換済みHTMLを格納し、Model経由でテンプレートへ渡しています。
...
テンプレート側では、Controllerから渡された変換結果のみを表示するように修正しました。

```html
<div class="card-text"
     th:utext="${answerHtmlMap[answer.id]}">
</div>
```

### 対応結果

* Thymeleafテンプレート解析エラーを解消
* 回答本文のMarkdown表示を正常化
